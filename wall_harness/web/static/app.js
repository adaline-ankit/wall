...
function renderFocus() {
  const current = state.entries.filter((entry) => entry.status === "inbox" || entry.status === "kept").slice(0, 3);
...
async function openEntry(entryId) {
  try {
    ...
    $("#entry-detail").innerHTML = `<div class="entry-detail-head"><p class="entry-meta"><span>${escapeHTML(entry.origin)}</span><span>${escapeHTML(entry.source)}</span></p><h2 id="entry-dialog-title">${escapeHTML(entry.title)}</h2>${entry.url ? `<a class="source-url" href="${escapeHTML(entry.url)}" target="_blank" rel="noreferrer">Open original ↗</a>` : ""}<p>${escapeHTML(entry.summary || "Add why you saved this before the context fades.")}</p></div><div class="detail-actions"><button class="status-control ${entry.status === "reading" ? "is-active" : ""}" type="button" data-entry-status="reading" aria-pressed="${entry.status === "reading"}">${entry.status === "reading" ? "Read" : "Mark as read"}</button><button class="status-control ${entry.status === "kept" ? "is-active" : ""}" type="button" data-entry-status="kept" aria-pressed="${entry.status === "kept"}">Keep</button><button class="status-control ${entry.status === "archived" ? "is-active" : ""}" type="button" data-entry-status="archived" aria-pressed="${entry.status === "archived"}">Archive</button><button class="text-link" type="button" data-entry-draft="${entry.id}">Write from this ↗</button></div><section class="detail-section"><h3>Your margin</h3>${notes}${highlights}<form id="note-form"><label class="field"><span>Add a note</span><textarea name="body" required maxlength="50000" placeholder="What is the point worth keeping?"></textarea></label><button class="button button-secondary" type="submit">Add note</button></form><form id="highlight-form" class="highlight-form"><label class="field"><span>Save a highlight</span><textarea name="quote" required maxlength="10000" placeholder="A line you want to be able to find again."></textarea></label><label class="field"><span>Your thought <small>optional</small></span><input name="note" maxlength="10000" placeholder="Why this line matters" /></label><button class="text-link" type="submit">Keep this highlight ↗</button></form></section><section class="detail-section"><h3>Next action</h3>${tasks}<form id="task-form" class="inline-form"><label class="sr-only" for="task-title">Task</label><input id="task-title" name="title" required maxlength="500" placeholder="Turn this into a next step" /><button class="button button-secondary" type="submit">Add task</button></form></section>`;
...
async function setEntryStatus(status) {
  ...
  try {
    ...
    const notice = status === "reading"
      ? "Marked as read. It stays in your library and out of today's queue."
      : status === "kept"
        ? "Kept for future you."
        : "Moved out of your active library.";
    showNotice(notice);
  } catch (error) {
    showNotice(error.message, true);
  }
...
$("#entry-dialog").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (form.id === "highlight-form") await request(`/api/reading/entries/${state.activeEntry.id}/highlights`, { method: "POST", body: JSON.stringify({ quote: new FormData(form).get("quote"), note: new FormData(form).get("note") || "" }) });
